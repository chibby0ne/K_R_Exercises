/* 
Filename: Exercise8-6.c
Author: Antonio Gutierrez
Description: The standard library function calloc(n, size) return a pointer to n objects of size size, with the storage initialized to zero. Write calloc, by calling malloc or by modifying it.
...
The variable base is used to get started. If f reep is NULL, as it is at the
...
search for a free block of adequate size begins at the point (f reep) where the
