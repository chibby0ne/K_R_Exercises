...
char *alloc(int n);
void getfield(char *p, char **pp, char **qq, int field);
char *findtab(char *s, int field);
...
int charcmp(char *s1, char *s2)
{
    ...
    if (field)
    {
        getfield(s1,&p1,&q1,field);
        getfield(s2,&p2,&q2,field);

        do{
            if (directory)
            {
                while (!isalnum(tolower(*p1)) && tolower(*p1)!=' ' && p1!=q1)
                    p1++;
                while (!isalnum(tolower(*p2)) && tolower(*p2)!=' ' && p2!=q2)
                    p2++;
            }
